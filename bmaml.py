...
class Bmaml(object):
	...
	def __init__(self, 
		...
		self.num_meta_updates_print = int(100/minibatch_size)
		self.num_epochs = num_epochs
		self.expected_total_tasks_per_epoch = 1000
		...
		for _ in range(self.num_particles):
			...
			for key in self.w_shape.keys():
				if isinstance(self.w_shape[key], tuple):
					theta_temp = torch.empty(self.w_shape[key], device = 'cpu')
					torch.nn.init.xavier_normal_(tensor=theta_temp)
				else:
					theta_temp = torch.zeros(self.w_shape[key], device = 'cpu')
	...
	def get_task_prediction(self, x_t, y_t, x_v, y_v=None):
		'''
		If y_v is not None:
			this is training
			return NLL loss
		...
		meta_weight = get_weights_target_net(w_generated=q_chaser, row_id=particle_id, w_target_shape=self.w_shape)
		y_pred = self.net.forward(x=x_v, w=meta_weight, p_dropout=self.p_dropout_base)
	...
	def get_pairwise_distance_matrix(self, particle_tensor):
		...
		pairwise_d_matrix = torch.zeros((num_particles, num_particles), device='cpu')
	...
	def meta_validation(self, datasubset, num_val_tasks, return_uncertainty=False):
		x0 = torch.linspace(start=-5, end=5, steps=100, device='cpu').view(-1, 1) # vector

		quantiles = np.arange(start=0., stop=1.1, step=0.1)
		cal_data = []

		data_generator = DataGenerator(num_samples=self.num_training_samples_per_class, device='cpu')
